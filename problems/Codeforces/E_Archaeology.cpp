...
int main() {
  ...
  cin.tie(nullptr);
  int n; cin >> n;
  vector<vector<pair<int,int>>> adj(n + 1); // adj[node] -> [(adjN, adjW), ...]
  for (int i = 0; i < n - 1; i++) {
    int a, b, c; cin >> a >> b >> c;
    adj[a].push_back({b, c});
    adj[b].push_back({a, c});
  ...
  multiset<pair<int,int>> ms; // holds (dfsTraversalOrder, node)
  ll currSize = 0;

function<void(int,int)> add = [&](int time, int node) {
  auto it = ms.insert({time, node});
  int sz = (int)ms.size();
  if (sz == 1) return;

  auto L = (it == ms.begin() ? prev(ms.end()) : prev(it));
  auto R = (next(it) == ms.end() ? ms.begin() : next(it));

  currSize += lift.pathDistWeighted(L->second, node);
  currSize += lift.pathDistWeighted(node, R->second);
  currSize -= lift.pathDistWeighted(L->second, R->second);
};

function<void(int,int)> remove = [&](int time, int node) {
  auto it = ms.find({time, node});
  if (it == ms.end()) return;

  int sz = (int)ms.size();
  if (sz == 1) { ms.erase(it); currSize = 0; return; }
