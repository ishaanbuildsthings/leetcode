...
class LazyPropagationSegmentTree:
    ...
    def _push(self, i, tl, tr):
        if self.lazy[i] is not None:
            ...
            right_child = 2 * i + 1
            nodeWidth = tr - tl + 1
            self.tree[i] = self._applyAggregate(self.lazy[i], self.tree[i], nodeWidth)
            if tl != tr:
...
# each node stores max prefix, max suffix, max subsegment, total sum
def baseFn(v, i):
    return v, v, v, v

def combine(a, b, c, d, e, f):
    # print(f'combine called on left={a} right={b}')
    leftP, leftS, leftAns, leftSum = a
    rightP, rightS, rightAns, rightSum = b

    # new prefix is either left prefix or leftSum + rightPrefix
    newPref = max(leftP, leftSum + rightP)
    # new suffix is either right suffix or rightSum + leftSuffix
    newSuff = max(rightS, rightSum + leftS)
    # new sum is just the two
    newSum = leftSum + rightSum

    # max subsegment is either of the two, or left suff max and right pref max
    maxAns = max(leftAns, rightAns, leftS + rightP)

    return newPref, newSuff, maxAns, newSum

# assigning overwrites the segment
def applyLazyToVal(lazy, val, nodeWidth):
    return (lazy * nodeWidth, lazy * nodeWidth, lazy * nodeWidth, lazy * nodeWidth)

def combineLazies(oldLazy, newLazy):
    return newLazy
...
st = LazyPropagationSegmentTree(arr, baseFn, combine, applyLazyToVal, combineLazies)
for _ in range(m):
    # print('-------------------------')
    l, r, v = map(int, input().split())
    r -= 1
    # print(f'assigning all elements from {l}:{r} to be {v}')
    st.updateRange(l, r, v)
    print(max(0, st.tree[1][2]))
    # print(st.query(0, len(arr) - 1)[2])