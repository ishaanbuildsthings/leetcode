...
for _ in range(T):
    ...
    dp = [0] * (g1 + 1) # dp[size] is how many ways we can form that group
    dp[0] = 1

    for item in arr:
        ndp = dp[:]
        for oldWeight in range(tot + 1):
            newWeight = item + oldWeight
            if newWeight > g1:
                break
            ndp[newWeight] += dp[oldWeight]
        dp = ndp

    ways = dp[g1]
    ways *= calc.getFactorialWithMod(g1)
    ways *= calc.getFactorialWithMod(g2)
    ways %= MOD
    for v in arr:
        ways *= calc.modInv(calc.getFactorialWithMod(v))
        ways %= MOD
    print(ways % MOD)
