...
int main() {
  ...
  for (int i = 0; i < m; i++) {
    ...
    int pathDist = lift.pathDist(a, b);
    // cout << "PATH DIST: " << pathDist << endl;
    if (pathDist % 2) {
      cout << 0 << endl;
      continue;
    }
    if (depths[a] < depths[b]) swap(a, b); // a is deeper
    int median = lift.kthOnPath(a, b, (pathDist / 2) + 1);
    // cout << "MEDIAN " << median << endl;
    int ab = lift.lca(a, b);
    int subtreeSize = sz[median];
    int result = subtreeSize;
    // cout << "subtree size init result: " << result << endl;
    // in general we will score the entire subtree size for our median
    // but don't count anything along the path a<>b, so remove the (median->a, 2) subtree
    result -= sz[lift.kthOnPath(median, a, 2)];
    // if we are the LCA, remove the b side as well
    // but then we get to add everything outside the entire subtree
    if (ab == median) {
      result -= sz[lift.kthOnPath(median, b, 2)];
      int outsideMedianTree = n - sz[median];
      result += outsideMedianTree;
    }
    cout << result << endl;
  }
