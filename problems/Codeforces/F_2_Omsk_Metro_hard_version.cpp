...
//     return {v, e, e, e};
// };
// auto merge = [&](Sub L, Sub R) -> Sub {          // L is the LEFT part (closer to A) on the path
...
// auto lifter = makeLift(0, edges, vals, base, merge, reverse, false);
...
struct Lift {
    ...
    Lift(int root, vector<pair<int,int>>& edges, vector<V>& vals,
         BaseFn baseFn, MergeFn mergeFn, ReverseFn reverseFn, bool zeroIndexed = true)
...
auto base = [](int v) -> Node {
    return Node{max(0, v), max(0, v), max(0, v), min(0, v), min(0, v), min(0, v), v};
};

auto mergeFn = [](Node L, Node R) -> Node {
    ll npfMax = max(L.pfMax, L.tot + R.pfMax);
    ll nsuffMax = max(R.suffMax, R.tot + L.suffMax);
    ll ntot = L.tot + R.tot;
...
auto rev = [](Node x) -> Node {
    return {x.suffMax, x.pfMax, x.subMax, x.suffMin, x.pfMin, x.subMin, x.tot};
};
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (qtype == '?') {
            ...
            int prev, val; cin >> prev >> val;
            edges.push_back({prev, nextNode++});
            vals.push_back(val);
        }
    }
    auto lifter = makeLift(1, edges, vals, base, mergeFn, rev, false);
    for (auto& q : queries) {
        Node out = lifter.pathQuery(q[0], q[1]);
        if (out.subMin <= q[2] && out.subMax >= q[2]) {
            cout << "YES" << '\n';
        } else {
            cout << "NO" << '\n';
        }
