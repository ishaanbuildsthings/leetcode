...
# Got ~850ms on codeforces circular RMQ: https://codeforces.com/problemset/submission/52/322591493
...
# Point assign - O(logN)
# Query point - O(logN)
# Leftmost index LTE X in a range - O(logN)
# Rightmost index LTE X in a range - O(logN)
# Find leftmost index of min element in a range - O(logN)
# Find rightmost index of min element in a range - O(logN)
...
class RangeAddRangeMin:
    ...
    def __init__(self, arr):
        ...
        for i in range(self.N - 1, 0, -1):
            tree[i] = fmin(tree[i << 1], tree[i << 1 | 1])
    ...
    def _pullUp(self, i):
        ...
        i >>= 1
        while i:
            tree[i] = fmin(tree[i << 1], tree[i << 1 | 1]) + lazyAdd[i]
            i >>= 1
    ...
    def rangeAdd(self, ql, qr, val):
        ...
        # Don't understand the logic here of why we only pull up the two endpoints but it is likely similar to what we do in queryMin logic explanation
        self._pullUp(l0)
        self._pullUp(r0)

    # Returns the minimum value in the range [ql, qr], O(logN)
    ...
    def printDiagram(self):
        ...
        def _print(node, l, r, prefix, isLast):
            ...
            print(f"{prefix}{connector}[{l}, {min(r, n-1)}]  "
                  f"v={tree[node] if tree[node] != INF else 'INF'}  "
                  f"{lazyStr}")
    ...
    # UNOPTIMIZED
    # O(logN)
    def leftmostIdxLTEX(self, ql, qr, val):
        if ql > qr:
            return -1
        return self._leftmostIdxLTEXHelper(1, 0, self.N, ql, qr + 1, val)

    def _leftmostIdxLTEXHelper(self, node, tl, tr, ql, qr, val):
        # No intersection
        if qr <= tl or tr <= ql:
            return -1

        # Check if this segment's minimum is greater than val
        current_min = self.tree[node]
        if current_min > val:
            return -1
        ...
        leftResult = self._leftmostIdxLTEXHelper(node << 1, tl, tm, ql, qr, val)
        if leftResult != -1:
            return leftResult

        # Try right child
        return self._leftmostIdxLTEXHelper(node << 1 | 1, tm, tr, ql, qr, val)

    # Find rightmost index in [ql, qr] where element <= val, returns -1 if not found, recursive walk
    # UNOPTIMIZED
    # O(logN)
    def rightmostIdxLTEX(self, ql, qr, val):
        if ql > qr:
            return -1
        return self._rightmostIdxLTEXHelper(1, 0, self.N, ql, qr + 1, val)

    def _rightmostIdxLTEXHelper(self, node, tl, tr, ql, qr, val):
        # No intersection or min in this segment > val
        ...
        rightResult = self._rightmostIdxLTEXHelper(node << 1 | 1, tm, tr, ql, qr, val)
        if rightResult != -1:
            return rightResult

        # Try left child
        return self._rightmostIdxLTEXHelper(node << 1, tl, tm, ql, qr, val)

    # Find leftmost index of minimum element in a range, returns -1 if not found, recursive walk
    # UNOPTIMIZED (can maybe do 1 logN call instead of 2?)
    # O(logN)
    def findMinIndexLeft(self, ql, qr):
        if ql > qr:
            return -1
        # First find the minimum value in the range
        minVal = self.queryMin(ql, qr)
        # Then find the leftmost index with that value
        return self.leftmostIdxLTEX(ql, qr, minVal)

    # Find rightmost index of minimum element in a range, returns -1 if not found
    # UNOPTIMIZED (can maybe do 1 logN call instead of 2?)
    # O(logN)
    def findMinIndexRight(self, ql, qr):
        if ql > qr:
            return -1
        # First find the minimum value in the range
        minVal = self.queryMin(ql, qr)
        # Then find the rightmost index with that value
        return self.rightmostIdxLTEX(ql, qr, minVal)
