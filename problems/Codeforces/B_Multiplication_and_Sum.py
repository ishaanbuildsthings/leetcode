...
def combine(a, b, c, d, e, f):
    return (a + b) % M
def applyLazy(lazy, current):
    return (lazy * current) % M
def combineLazies(oldLazy, newLazy):
    return (oldLazy * newLazy) % M

arr = [1] * n
st = LazyPropagationSegmentTree(arr, baseFn, combine, applyLazy, combineLazies)
...
for _ in range(m):
    ...
    if query[0] == 1:
        l, r, v = query[1:]
        r -= 1
        # print(f'multiplying from {l} to {r} by {v}')
        st.updateRange(l, r, v)
    else:
        l, r = query[1:]
        r -= 1
        # print(f'querying from {l} to {r}')
        print(st.query(l, r))