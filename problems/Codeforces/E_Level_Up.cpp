...
int main() {
    ...
    vector<pair<int, int>> qs(q); // holds (i, x)
    for (int j = 0; j < q; j++) {
        cin >> qs[j].first >> qs[j].second;
    }

    MergeSortTree mst(A);

    // answers[k] holds ranges (l, r, strength) meaning anyone in l...r with strength >= level was fought
    vector<vector<tuple<int, int, int>>> answers(n + 1);

    for (int k = 1; k <= n; k++) {
        int pos = 0, level = 1;
        vector<tuple<int, int, int>> ranges;
        while (pos < n) {
            int nextI = mst.findKthGteX(pos, n - 1, k, level);
            if (nextI == -1) {
                ranges.push_back({pos, n - 1, level});
                break;
            }
            ranges.push_back({pos, nextI, level});
            pos = nextI + 1;
            level++;
        }
        answers[k] = ranges;
    ...
    for (auto [i, x] : qs) {
        i--;
        auto& ranges = answers[x];
        // find the range containing index i
        int rangeIdx = (int)(upper_bound(ranges.begin(), ranges.end(), make_tuple(i, INT_MAX, INT_MAX)) - ranges.begin()) - 1;
        auto [l, r, strength] = ranges[rangeIdx];
        cout << (A[i] >= strength ? "YES" : "NO") << '\n';
    }
