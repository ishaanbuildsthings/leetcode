...
def agg(a, b):
    return max(a, b)
def applyLazy(lazy, data, segLen):
    return lazy + data
def composeLazies(old, newL):
    return old + newL
hld = LazyHLD(n, edges, vals, base, agg, applyLazy, composeLazies)
for _ in range(q):
    ...
    if qtype == 'add':
        ...
        node -= 1
        delta = arr[2]
        hld.subtreeApply(node, delta)
    else:
        a = arr[1]
        b = arr[2]
        a -= 1
        b -= 1
        print(hld.pathQuery(a, b))
