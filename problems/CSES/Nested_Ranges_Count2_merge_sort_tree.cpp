...
int main() {
    ...
    sort(ranges.begin(), ranges.end(), [](Range& range1, Range& range2) {
    ...
    MergeSortTree mst(rs);

    vector<int> containsOther(n, 0);
    vector<int> containsMe(n, 0);

    for (int i = 0; i < n; i++) {
        Range& range = ranges[i];
        // find leftmost with an L >= our range's L, so we contain it
        int L = 0;
        int R = n - 1;
        int resI = -1;
        while (L <= R) {
        ...
        // find rightmost with an L <= our range's R, so we contain it
        L = 0;
        R = n - 1;
        int resI2 = -1;
        while (L <= R) {
        ...
        int countWeContain = mst.countLteX(resI, resI2, range.r);
        containsOther[range.idx] = countWeContain;

        // find rightmost range with an L <= our range's L, so it can contain us
        L = 0;
        R = n - 1;
        int resI3 = -1;
        while (L <= R) {
        ...
        int countContainMe = mst.countGteX(0, resI3, range.r);
        containsMe[range.idx] = countContainMe;
    }

    for (auto x : containsOther) cout << x - 1 << " ";
    cout << endl;
    for (auto x : containsMe) cout << x - 1 << " ";
    cout << endl;
}