...
calc = ModCalc(10**5 + 10, 10**9 + 7)
class Solution:
    def countGoodArrays(self, n: int, m: int, k: int) -> int:
        # continuous blocks = n - k
        # inner partitions required = blocks - 1
        # possible partition slots = n - 1
        # ways to place inner partitions = nChooseK(possible slots, required positions)
        # possible number types = m^continuous blocks
        
        continuousBlocks = n - k
        innerPartitionsRequired = continuousBlocks - 1 # how many we place
        possiblePartitionSlots = n - 1
        possibleNumberTypes = calc.modMultiply(calc.modPow(m - 1, continuousBlocks - 1), m)
        waysToPlacePartitions = calc.nChooseKWithMod(possiblePartitionSlots, innerPartitionsRequired)
        return calc.modMultiply(possibleNumberTypes, waysToPlacePartitions)
