class Solution:
    def largestRectangleArea(self, heights: List[int]) -> int:
      ...
      # two stacks, find the first on left < height, and first on right < height
        w = len(heights)
        leftSmaller = [-1] * w
        stack = [] # strict-increasing
        for c in range(w):
            height = heights[c]
        ...
        for c in range(w - 1, -1, -1):
            ...
            while stack and height <= heights[stack[-1]]:
                stack.pop()
            right = stack[-1] if stack else w
            rightSmaller[c] = right
            stack.append(c)
        res = 0
        for c in range(w):
            left = leftSmaller[c] + 1
            right = rightSmaller[c] - 1
            res = max(res,(right - left + 1) * heights[c])
        return res
