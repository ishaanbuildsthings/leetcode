...
class Solution:
    def maxProfit(self, prices: List[int], profits: List[int]) -> int:
        ...
        for price, profit in zip(prices, profits):
            ...
            prevBest = dp2.queryMax(0, price - 1)
            nTriple = prevBest + profit
            res = max(res, nTriple)

            # we could stack this onto 1 existing purchase to make 2
            prevSingle = dp1.queryMax(0, price - 1)
            nDouble = prevSingle + profit
            currDouble = dp2.queryMax(price, price)
            if nDouble > currDouble:
                dp2.pointUpdateAndMutateArray(price, nDouble)

            # we could buy this outright as one
            curr = dp1.queryMax(price, price)
            if profit > curr:
                dp1.pointUpdateAndMutateArray(price, profit)
