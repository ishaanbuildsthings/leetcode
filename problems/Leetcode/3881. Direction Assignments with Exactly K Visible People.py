...
MOD = 10**9 + 7
calc = ModCalc(10**5 + 5, MOD)
class Solution:
    def countVisiblePeople(self, n: int, pos: int, k: int) -> int:
        leftOpts = pos
        rightOpts = n - 1 - leftOpts
        # print(f'{rightOpts=}')
        res = 0
        for lefts in range(k + 1):
            rights = k - lefts

            waysSelectLeft = calc.nChooseKWithMod(leftOpts, lefts)
            waysRight = calc.nChooseKWithMod(rightOpts, rights)
            nways = waysSelectLeft * waysRight
            nways %= MOD
            res += nways
            res %= MOD
