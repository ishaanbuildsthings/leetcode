...
class MaxSegTree:
    ...
    def pointChmax(self, index, v):
        if self.arr[index] >= v:
            return
        self.pointUpdateAndMutateArray(index, v)
...
class Solution:
    def maxAlternatingSum(self, nums: list[int], k: int) -> int:
        ...
        nxtUp = MaxSegTree([0] * len(comp)) # max score for a subsequence ending in this value
        nxtDown = MaxSegTree([0] * len(comp)) # max score for a subsequence ending in this value
        updates = defaultdict(list) # holds (update type, new value)
        for i, v in enumerate(nums):
            compressed = comp[v]
            for type, newVal in updates[i]:
                if type == 1:
                    nxtUp.pointChmax(compressed, newVal)
                else:
                    nxtDown.pointChmax(compressed, newVal)

            # also we can always use ourselves
            nxtUp.pointChmax(compressed, v)
            nxtDown.pointChmax(compressed, v)
            
            # after applying the i-th operation, my segment tree is in a good state to answer some future state
            # after applying the i-th operation, I hold the answer for cell i+k

            futureI = i + k
            if futureI < n:
                futureV = nums[futureI]
                futureCompressed = comp[futureV]
                ifFutureUp = nxtUp.queryMax(0, futureCompressed - 1) + futureV
                ifFutureDown = nxtDown.queryMax(futureCompressed + 1, len(comp) - 1) + futureV
                updates[futureI].append((1, ifFutureDown))
                updates[futureI].append((2, ifFutureUp))
            
        ans = max(nxtUp.queryMax(0, len(comp) - 1), nxtDown.queryMax(0, len(comp) - 1))
        return ans
