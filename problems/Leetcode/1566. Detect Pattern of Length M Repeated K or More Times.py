...
class Solution:
    def containsPattern(self, arr: List[int], m: int, k: int) -> bool:
        n = len(arr)
        h = PalindromeHashing(arr,911,10**9+7, lambda x : x)
        # given a starting index L, tells us the furthest right ending index R where we can get repeated subarrays of size M
        goRight = {}
        goRight[n-m+1] = n - 1
        n = len(arr)
        for L in range(n - m, -1, -1):
            nxt = L + m
            nextBlockFits = nxt + m - 1 <= n - 1
            if nextBlockFits and h.getHashForSubstring(L, L + m - 1) == h.getHashForSubstring(nxt, nxt + m - 1):
                goRight[L] = goRight[nxt]
            else:
                goRight[L] = L + m - 1

            if (goRight[L] - L + 1) // m >= k:
                return True
