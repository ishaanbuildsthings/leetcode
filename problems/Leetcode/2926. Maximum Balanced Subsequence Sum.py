...
class MaxSegTree:
    ...
    def pointChmax(self, index, newVal):
        if self.arr[index] >= newVal:
            return
        self.pointUpdateAndMutateArray(index, newVal)
...
class Solution:
    def maxBalancedSubsequenceSum(self, nums: List[int]) -> int:
        ...
        st = MaxSegTree([-inf] * len(comp))
        for i, v in enumerate(nums):
            diff = v - i
            compressed = comp[diff]
            pMax = st.queryMax(0, compressed)
            nmax = max(v, pMax + v)
            st.pointChmax(compressed, nmax)
        return st.queryMax(0, len(comp) - 1)
