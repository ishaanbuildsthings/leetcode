class StaticDistinct:
    ...
    # Values may be any hashable type — nxt holds indices, so no compression.
    def __init__(self, arr):
        n = len(arr)
        nxt = [n] * n
        last = {}
        for i in range(n - 1, -1, -1):
            nxt[i] = last.get(arr[i], n)
            last[arr[i]] = i
        self.mst = MergeSortTree(nxt)

    # O(log n) — # of distinct values in a[l..r], inclusive, 0-indexed
    def countDistinct(self, l, r):
        return self.mst.countGteX(l, r, r + 1)
...
class Solution:
    def validSubarrays(self, nums: List[int], k: int, l0: int, r0: int, q: int) -> List[bool]:
        n = len(nums)
        res = []
        st = StaticDistinct(nums)
        pf = []
        curr = 0
        mp = {} # number to random number
        seen = set() # seen randoms
        for v in nums:
            if v in mp:
                curr ^= mp[v]
                pf.append(curr)
        ...
        for _ in range(q):
            ...
            xor = queryXor(l, r)
            if xor == 0:
                distinct = st.countDistinct(l, r)
                if distinct == k:
                    answer = True
            res.append(answer)
            gi = l + r if answer else r - l
            nl = (l ^ gi) % n
            nr = (r ^ gi) % n
