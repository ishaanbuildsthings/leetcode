...
public:
    long long maxSum(vector<int>& nums, int k) {
        int n = nums.size();
        ll res = *min_element(nums.begin(), nums.end());
        OrderStatisticFenwick inside(nums);
        OrderStatisticFenwick outside(nums);
        for (int L = 0; L < n; L++) {
            inside.clear();
            outside.clear();
            // OrderStatisticFenwick inside(nums);
            ...
            if (outside.size() && outside.kth(outside.size()) > inside.kth(1) && k) {
                swappedIn += outside.kth(outside.size());
                swappedOut += inside.kth(1);
                swaps++;
            }

            ll TOT = nums[L];

            ll score = nums[L] - swappedOut + swappedIn;
            res = max(res, score);
            ...
            for (int R = L + 1; R < n; R++) {
                ...
                TOT += v;
                outside.remove(v);
                ll outsideBigger = outside.countGTX(v);
                if (outsideBigger < swaps) {
                    swappedIn -= v;
                    if (outside.size() >= swaps) {
                        swappedIn += outside.kth(outside.size()-swaps+1);
                        
                    } else {
                        swappedOut -= inside.kth(swaps);
                        swaps--;
                    }
                }

                ll idx = inside.countLTX(v);
                inside.add(v);
                if (idx < swaps) swappedOut += (v - inside.kth(swaps+1));

                int bottle = min(inside.size(), outside.size());
                bottle = min(bottle, k);


                if (swaps) {
                    if ((swaps>bottle) || (outside.kth(outside.size() - swaps + 1) - inside.kth(swaps) <= 0)) {
                        swappedOut-=inside.kth(swaps);
                        swappedIn -= outside.kth(outside.size()-swaps+1);
                        swaps--;
                    }
                ...
                if (swaps < bottle && outside.kth(outside.size()-swaps) - inside.kth(swaps+1) > 0) {
                    swappedOut += inside.kth(swaps+1);
                    swappedIn += outside.kth(outside.size()-swaps);
                    swaps++;
                }

                score = TOT - swappedOut + swappedIn;
                res = max(res, score);
            }
