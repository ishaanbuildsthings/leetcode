...
class Solution:
    def minimumGroups(self, words: List[str]) -> int:
        ...
        # even hash + odd hashes -> group id


        mp = {} # (evenHash, oddHash) -> groupId
        ...
        for w in words:
            evenLetters = ''.join([c for i, c in enumerate(w) if i % 2 == 0])
            oddLetters = ''.join([c for i, c in enumerate(w) if i % 2 == 1])

            h1 = IncrementalHashing(evenLetters,911,GM)
            h2 = IncrementalHashing(oddLetters,911,GM)
            ...
            allTups = [(evenHash, oddHash)]

            # try fixed even
            for i in range(len(oddLetters) - 1, -1, -1):
                c = oddLetters[i]
                h2.popChar()
                h2.addCharLeft(c)

                allTups.append((evenHash, h2.getHash()))

            # try fixed odd
            for i in range(len(evenLetters) - 1, -1, -1):
                c = evenLetters[i]
                h1.popChar()
                h1.addCharLeft(c)

                allTups.append((h1.getHash(), oddHash))
