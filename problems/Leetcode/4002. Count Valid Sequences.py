...
class Solution:
    def countValidSequences(self, n: int, k: int) -> int:
        calc = ModCalc(n + 100, MOD)

        overs = n - k

        ways = calc.waysToPutIdenticalItemsIntoDistinctBucketsWithMod(overs, k)
        # print(f'{ways=}')

        # but at least one of the assignments must be odd

        # any assignment where everything got evens is bad

        if overs % 2:
            # print(f'overs is mod 2, ret ways')
            return ways


        halves = overs // 2
        # print(f'halves={halves}')
        w2 = calc.waysToPutIdenticalItemsIntoDistinctBucketsWithMod(halves, k)
        # print(f'half assigns: {halves}')
        # print(f'ways2: {w2}')
        ...
        # dp[totalLeft][usedEven]

        # we can loop over all numbers and add them

        # pf sum dp



        # even product means includes at least one even

        # odd products means odds only



        # n=5 k=3 means we have overage of 2 to assign

        # we can assign these to many numbers, as long as one of them as an odd amount of assignments
