...
class Solution:
    def distinctPaths(self, n: int, parent: list[int], gates: list[list[int]], queries: list[list[int]]) -> int:
        ...
        # vals hold (bb, br, rr, rb) meaning ways to get from node 1 to node 2 starting with a blue card ending with a red card for example
        vals = [(b, w, r, w) for r, b, w in gates]
        base = lambda x: x
        def merge(v1, v2):
            nbb = (v1[0]*v2[0] + v1[1]*v2[3]) % MOD
            nbr = (v1[0]*v2[1] + v1[1]*v2[2]) % MOD
            nrr = (v1[2]*v2[2] + v1[3]*v2[1]) % MOD
            nrb = (v1[3]*v2[0] + v1[2]*v2[3]) % MOD
            return (nbb, nbr, nrr, nrb)

        lift = Lift(0, edges, vals, base, merge)
        
        res = 0

        def ways(node, card, lca):
            steps = lift.depths[node] - lift.depths[lca]
            if steps == 0:
                return 1
            bb, br, rb, rr = lift.liftQuery(node, steps)
            return (bb + br) % MOD if card == 0 else (rb + rr) % MOD

        res = 0
        for anode, acard, bnode, bcard in queries:
            lca = lift.lca(anode, bnode)
            res ^= ways(anode, acard, lca) * ways(bnode, bcard, lca) % MOD
        return res
