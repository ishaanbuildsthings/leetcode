...
class IncrementalHashing:
    ...
    def swapCharAt(self, index: int, newChar: str):
        ...
        oldCoefficient = ord(oldChar)
        newCoefficient = ord(newChar)
...
class StreamChecker:
    ...
    def __init__(self, words: List[str]):
        sizes = set([len(w) for w in words])
        self.sizeToHash = {
            size : IncrementalHashing('',mod=10**9+7) for size in sizes
        }
        self.targetHashes = set([IncrementalHashing(w,mod=10**9+7).getHash() for w in words])

    def query(self, letter: str) -> bool:
        res = False
        for size in self.sizeToHash:
            hasher = self.sizeToHash[size]
            if hasher.length() < size:
                hasher.addChar(letter)
            else:
                hasher.slideRight(letter)
            if hasher.getHash() in self.targetHashes:
                res = True
        return res
