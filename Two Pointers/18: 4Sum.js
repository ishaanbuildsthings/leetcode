...
However, in twoSum, we also want to skip duplicate values, but we are using a while loop. We could in theory use a for loop, and say for (let i = ...; i < r; i++) and modify r while inside the for loop. If we did that, we could use `continue`. But since we used a while loop, we can do something like:
l++
