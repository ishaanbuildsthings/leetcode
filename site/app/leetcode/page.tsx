...
export default async function LeetCodePage() {
  ...
  const problemsData = await problemService.unsafe_listProblemsByPlatformAndTags(
    "leetcode",
    TAG_SLUGS_ORDERED
  );
  ...
  for (const [slug, bucket] of grouped) {
    bucket.sort((a, b) => {
      const aDiff = a.tags.find((t) => t.tag.slug === slug)?.tagDifficulty ?? 99;
      const bDiff = b.tags.find((t) => t.tag.slug === slug)?.tagDifficulty ?? 99;
      return aDiff - bDiff;
    });
  }
