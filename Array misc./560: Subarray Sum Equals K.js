// Problem: Find number of subarrays that equal sum, k

// Solution ONLY if all numbers are positive: O(n) time and O(1) space. Uses a sliding window, we increment the window to the right when the sum is too small, and decrement it from the left when the sum is too big. This finds all solutions even if the numbers are not sorted. We could also keep decrementing until the sum is small enough, and always increment r.
// The sliding window works because we are only checking contiguous arrays. If the sum is too small, clearly  we need to add another number. But if it's too big, why can we just remove the left number? Consider this array: [1, 2, 3] and say we are pointing at 1 and 3, and our array sum is too big. We could reset both pointers to 2 like how we do in the n^2 and n^3 solutions, but it would be redundant to check that. How could the array [2] possibly be correct? If it were, [1, 2] would have been too big, and we would have decremented 1 already. The array also doesn't need to be sorted, say [2, 1, 3] and we are pointing at 2 and 3. [1] could not possibly be a solution, or we would have decremeneted from [2, 1] already.
