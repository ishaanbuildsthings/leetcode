...
// Solution 1, serialization. O(n+m) time (with rolling hash) and O(n+m) space. Iterate over the main tree (any order) and serialize it by adding characters for the null leafs, to make sure the tree is appropriate distinct. We also have to add delimiters for each value, for instance 22 could be interpreted as two nodes each with value 2, or one node with value 22. A delimiter like 2|2 makes it clear. We take n+m time to serialize both the main tree and potential subtree. Then we iterate across n, doing m string checks using a rolling hash, so still n time (though n*m worst case). NOTE, we can showcase how serializing a tree and a subtree means the subtree will appear contiguously, I think it can be done by some sort of induction or recursive argument. I mean once we enter the subtree to serialize, we won't pop out until we exit that root.
