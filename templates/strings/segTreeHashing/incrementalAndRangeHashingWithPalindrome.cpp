...
// h.hashRangeReversed(l, r) -> ll   hash of window[l..r] read backwards, O(log n)
...
public:
    ...
    // O(log n) time
    ll hashRangeReversed(int l, int r) {
        if (l > r) return 0;
        int lo = cap + left + l;
        int hi = cap + left + r + 1;
        ll lrev = 0, rrev = 0;
        ...
        while (lo < hi) {
            int w = 1 << h;
            if (lo & 1) {
                lrev = (revHashes[lo] * basePow[llen] + lrev) % mod;
                llen += w;
                lo++;
            }
            if (hi & 1) {
                hi--;
                rrev = (rrev * basePow[w] + revHashes[hi]) % mod;
            }
            lo >>= 1; hi >>= 1; h++;
        }
        return (rrev * basePow[llen] + lrev) % mod;
    }

    // Is window[l..r] a palindrome
    // O(log n) time
    bool isPalindrome(int l, int r) {
        return hashRange(l, r) == hashRangeReversed(l, r);
    }

    // Do the two ranges hold the same string
    // O(log n) time
    bool rangesEqual(int l1, int r1, int l2, int r2) {
        if (r1 - l1 != r2 - l2) return false;
        return hashRange(l1, r1) == hashRange(l2, r2);
    }

    // Is the first range the reverse of the second
    // O(log n) time
    bool isReverseOf(int l1, int r1, int l2, int r2) {
        if (r1 - l1 != r2 - l2) return false;
        return hashRange(l1, r1) == hashRangeReversed(l2, r2);
    }
    ...
    // O(log n) time
    bool isPalindromeConcat(int l1, int r1, int l2, int r2) {
        int len1 = r1 >= l1 ? r1 - l1 + 1 : 0;
        int len2 = r2 >= l2 ? r2 - l2 + 1 : 0;
        ensureBasePow(max(len1, len2));
        ll fwd = (hashRange(l1, r1) * basePow[len2] + hashRange(l2, r2)) % mod;
        ll rev = (hashRangeReversed(l2, r2) * basePow[len1] + hashRangeReversed(l1, r1)) % mod;
        return fwd == rev;
    }
    ...
    // O(log^2 n) time
    int longestPalCenteredAroundI(int i) {
        int n = length();
        int lo = 0, hi = min(i, n - 1 - i);
        while (lo < hi) {
            int mid = (lo + hi + 1) / 2;
            if (isPalindrome(i - mid, i + mid)) lo = mid;
            else hi = mid - 1;
        }
        return 2 * lo + 1;
    }
    ...
    // O(log^2 n) time
    int longestPalCenteredAroundII(int i) {
        int n = length();
        if (i + 1 >= n || charAt(i) != charAt(i + 1)) return 0;
        int lo = 1, hi = min(i + 1, n - 1 - i);
        while (lo < hi) {
            int mid = (lo + hi + 1) / 2;
            if (isPalindrome(i - mid + 1, i + mid)) lo = mid;
            else hi = mid - 1;
        }
        return 2 * lo;
    }
...
private:
    ...
    vector<ll> revHashes; // same shape, but each node holds the hash of its range reversed
    ...
    void pointSet(int pos, char c) {
        ...
        while (i) {
            ...
            revHashes[i] = (revHashes[j+1] * levelPow[h] + revHashes[j]) % mod;
            h++;
            i >>= 1;
        }
    }
