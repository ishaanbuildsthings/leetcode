...
// Values may be negative. Sums are long long. Sentinels are LLONG_MAX/4 so they survive being
...
struct BeatsB {
    ...
    explicit BeatsB(const vector<ll>& arr) {
        n = (int)arr.size();
    ...
    void _build(int i, int tl, int tr, const vector<ll>& arr) {
        Node& nd = t[i];
        nd.addMn = nd.addOther = 0; nd.hasAssign = false;
        if (tl == tr) {
            ll v = arr[tl];
            nd.mn = v; nd.mn2 = POS_INF; nd.cntMn = 1; nd.tot = v; nd.sz = 1;
            return;
        }
        int tm = (tl + tr) / 2;
    ...
    void _pull(int i) {
        ...
        nd.hasAssign = false;
        nd.addMn = nd.addOther = 0;
    }

    void _applyAdd(int i, ll vMin, ll vOther) {
        Node& nd = t[i];
        nd.tot += vMin * nd.cntMn + vOther * (nd.sz - nd.cntMn);
        if (nd.mn2 != POS_INF) nd.mn2 += vOther;
        nd.mn += vMin;
        if (nd.hasAssign) nd.assignVal = nd.mn;
        else { nd.addMn += vMin; nd.addOther += vOther; }
    }

    void _applyAssign(int i, ll v) {
        Node& nd = t[i];
        nd.mn = v; nd.mn2 = POS_INF; nd.cntMn = nd.sz;
        nd.tot = v * nd.sz;
        nd.addMn = nd.addOther = 0;
    ...
    void _push(int i) {
        ...
        ll aMn = nd.addMn, aO = nd.addOther;
        if (aMn == 0 && aO == 0) return;
        ll mnc = min(t[l].mn, t[r].mn);
        _applyAdd(l, t[l].mn == mnc ? aMn : aO, aO);
        _applyAdd(r, t[r].mn == mnc ? aMn : aO, aO);
        nd.addMn = nd.addOther = 0;
    }

    void _chmax(int i, int tl, int tr, int ql, int qr, ll x) {
        if (qr < tl || ql > tr || x <= t[i].mn) return;
        if (ql <= tl && qr >= tr && x < t[i].mn2) { _applyAdd(i, x - t[i].mn, 0); return; }
        _push(i);
        int tm = (tl + tr) / 2;
    ...
    ll pointGet(int pos) { return rangeMin(pos, pos); }
};