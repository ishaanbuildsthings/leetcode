...
class BeatsChmin_PERFORMANT:
    ...
    def _pull(self, i):
        ...
        self.addMx[i] = 0; self.addOther[i] = 0

    def _applyAdd(self, i, vMax, vOther):
        self.tot[i] += vMax * self.cntMx[i] + vOther * (self.sz[i] - self.cntMx[i])
        ...
        if self.hasAssign[i]:
            ...
            self.addMx[i] += vMax; self.addOther[i] += vOther
    ...
    def _push(self, i):
        ...
        if self.hasAssign[i]:
            ...
            self.hasAssign[i] = False
            return
        aMx = self.addMx[i]; aO = self.addOther[i]
        if aMx == 0 and aO == 0:
            return
        mxc = self.mx[l] if self.mx[l] > self.mx[r] else self.mx[r]
        self._applyAdd(l, aMx if self.mx[l] == mxc else aO, aO)
        self._applyAdd(r, aMx if self.mx[r] == mxc else aO, aO)
        self.addMx[i] = 0; self.addOther[i] = 0
    ...
    def _chmin(self, i, tl, tr, ql, qr, x):
        ...
        if ql <= tl and qr >= tr and x > self.mx2[i]:
            self._applyAdd(i, x - self.mx[i], 0)
            return
        self._push(i)
        tm = (tl + tr) // 2
