// template by ISHAANBUILDSTHINGS github
// SEE THE SECOND CLASS AT THE BOTTOM "StaticDistinct"
...
// Range count distinct, no updates. O(n log n) build, O(log n) query.
// nxt[i] = next index j > i with a[j] == a[i], else n.
// A value is counted once per window, at its LAST occurrence: nxt[i] > r.
// nxt holds indices, so the merge sort tree is over 0..n regardless of what
// a[] contains — T may be any hashable type, no coordinate compression needed.
template <typename T>
struct StaticDistinct {
    MergeSortTree mst;

    StaticDistinct() {}
    StaticDistinct(const vector<T>& arr) {
        int n = arr.size();
        vector<int> nxt(n, n);
        ...
        mst = MergeSortTree(nxt);
    }

    // O(log n) — # of distinct values in a[l..r], inclusive, 0-indexed
    int countDistinct(int l, int r) {
        return mst.countGteX(l, r, r + 1);
    }

    // O(log n) — true iff a[l..r] has no repeated value
