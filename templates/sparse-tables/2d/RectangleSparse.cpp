...
struct RectangleSparseTable {
    ...
    RectangleSparseTable(const vector<vector<T>>& grid, Combine combine) : combine(combine) {
        ...
        int LOG_W = __lg(w) + 1;
        dp.assign(LOG_H, vector<vector<vector<T>>>(LOG_W));
        dp[0][0] = grid;
        for (int pw = 1; pw < LOG_W; pw++) {
            int width = 1 << pw;
            int halfW = width >> 1;
            int cols = w - width + 1;
            if (cols <= 0) break;
            dp[0][pw].assign(h,vector<T>(cols));
            for (int r = 0; r < h; r++) {
                for (int c = 0; c < cols; c++) {
                    dp[0][pw][r][c] = combine(dp[0][pw-1][r][c], dp[0][pw-1][r][c+halfW]);
                }
            }
        }
        for (int pw = 1; pw < LOG_H; pw++) {
            int height = 1 << pw;
            int halfH = height >> 1;
            int rows = h - height + 1;
            if (rows <= 0) break;
            dp[pw][0].assign(rows,vector<T>(w));
            for (int r = 0; r < rows; r++) {
                for (int c = 0; c < w; c++) {
                    dp[pw][0][r][c] = combine(dp[pw-1][0][r][c], dp[pw-1][0][r+halfH][c]);
                }
            }
        }
    ...
    T query(int r1, int r2, int c1, int c2) {
        ...
        int heightOffset = height - (1 << heightBit);
        int widthOffset = width - (1 << widthBit);
        return combine(
