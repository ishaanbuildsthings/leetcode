...
struct LiftEdgeReverse {
    ...
    // data over the k edges from `node` going up (edge above node first), in TOP-DOWN
    ...
    // the edge above the LCA is NOT on the path.  nullopt if A == B.  O(log N)
    optional<T> pathQuery(int a, int b) {
        int l = lca(a, b);
        int da = dep[a] - dep[l], db = dep[b] - dep[l];
        optional<T> left  = da > 0 ? liftQuery(a, da) : nullopt;   // a-arm edges, top-down
        optional<T> right = db > 0 ? liftQuery(b, db) : nullopt;   // b-arm edges, top-down
        optional<T> res = nullopt;
        if (left)  res = reverseFn(*left);                         // reverse a-arm: reads a -> L
        if (right) res = res ? optional<T>(mergeFn(*res, *right)) : right;   // then L -> b
        return res;
    }
};
