...
# hld = HldLazySegTreeEdges(edges, vals, base, mergeFn, applyLazy, composeLazies, 1, False)
...
# O(log N)
# subtreeQuery(nodeLabel) -> EdgeData, aggregated data of all edges in the subtree, returns None if a leaf


class HldLazySegTreeEdges:
    ...
    # returns None if a == b (no edges on the path)
    def pathQuery(self, a, b):
        ...
        if depth[a] > depth[b]:
            a, b = b, a
        if pos[a] + 1 <= pos[b]:
            res = self._combineOpt(res, self._segQuery(pos[a] + 1, pos[b]))
        return res
