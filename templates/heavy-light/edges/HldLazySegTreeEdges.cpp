...
// pathApply(a, b, lazyVal)              applies to all EDGES on path a..b
// subtreeApply(node, lazyVal)           applies to edges from node down to all descendants (NOT the edge above node)
// pathQuery(a, b)                       aggregated data over edges on path a..b. CALLER MUST ENSURE a != b
// subtreeQuery(node)                    aggregated data over edges from node down to all descendants. CALLER MUST ENSURE node has a descendant
//
...
// ----------------------------------------------------------
//   using RawT     = long long;
//   using EdgeData = long long;
//   using LazyTag  = long long;
//
...
//       return a + b;
//   };
//   auto applyLazy = [](LazyTag lz, EdgeData d, int segLen) -> EdgeData {
...
//   };
//
//   LazyHLDEdge<RawT, EdgeData, LazyTag> hld(n, edges, base, combine, applyLazy, composeLazies);
//   hld.pathApply(2, 3, 10);
//   EdgeData total = hld.pathQuery(2, 3);
// =============================================================================
...
struct LazyHLDEdge {
    ...
    EdgeData pathQuery(int a, int b) {
        ...
        if (depth[a] > depth[b]) swap(a, b);
        auto last = _segQuery(pos[a] + 1, pos[b]);
