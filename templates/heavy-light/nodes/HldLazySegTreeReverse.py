...
# hld = LazyHLD(numNodes, edgeList, vals, baseFn, combineFn, applyLazyFn, composeLaziesFn, reverseFn)
class LazyHLD:
    def __init__(self, n, edges, vals, base, combine, applyLazy, composeLazies, reverse):
        self.n = n
        self.base = base
    ...
    def pathQuery(self, a, b):
        ...
        if self.depth[a] >= self.depth[b]:
            chunk = self._segQuery(self.pos[b], self.pos[a])
            leftPieces.append(self.reverse(chunk))
        else:
            chunk = self._segQuery(self.pos[a], self.pos[b])
            rightPieces.append(self.reverse(chunk))
        leftAcc = None
        for p in leftPieces:
