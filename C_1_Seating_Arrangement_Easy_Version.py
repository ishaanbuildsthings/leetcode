...
def solve():
    ...
    for i, v in enumerate(s):
        ...
        if v == 'I':
            # print(f'seating I')
            if empty == 0:
                # print(f'no empty for I, kicking out')
                continue
            
            # if there was an empty but we would overflow with ambiverts, do not seat
            totalSpace = empty * seats
            total = fw.sum()
            cnt = len(fw)
            remainSpace = empty * seats + (cnt * seats - total) - savedA
            # for part in partials:
            #     totalSpace += seats - part
            # remainSpace = totalSpace - savedA
            if remainSpace < seats:
                # print(f'there was an empty but we overflow with ambiverts, cannot seats this I')
                continue
            
            # print(f'sat an introvert')
            res += 1
            empty -= 1
            fw.add(1)
            # partials.append(1)
            ...
            # if not partials:
            if len(fw) == 0:
                # print(f'no partials for E...')
                if savedA and empty:
                    # print(f'but we did have a savedA and at least one empty')
                    empty -= 1
                    savedA -= 1
                    # partials.append(2)
                    fw.add(2)
                    res += 2
                    continue
                # print(f'no partials and no savedA, kicking out')
                continue
            # print(f'seating extrovert at smallest table')
            # lost = partials.pop(0)
            lost = fw.kthSmallest(0)
            fw.remove(lost)
            nv = lost + 1
            if nv == seats:
                full += 1
    ...
    total = fw.sum()
    cnt = len(fw)
    remain = (cnt * seats) - total
    # bottle = (empty * seats) + sum((seats-x) for x in partials)
    bottle = (empty * seats) + remain
    bottle = min(bottle, savedA)
    # print(f'gaining last saved A')
    res += bottle
